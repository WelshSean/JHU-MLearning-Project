...
When we read in the data, we tidy up some of the mess including NA as a string, "DIV/0!" and null cells by using the na.strings argument to read.csv. After doing this, we see a few colums that contain nothing but NAs so we remove those and we convert classes to factors

```{r}
pmltraining <- pmltraining[, colSums(is.na(pmltraining)) != nrow(pmltraining)]
pmltraining$classe <- factor(pmltraining$classe)
pmltraining$new_window <- factor(pmltraining$new_window)
# Remove columns where number of NAs = nrows
```
...
```{r}
names(pmltraining)
columnsToRemove <- grep("^var.*|^stddev.*|^max.*|^min.*|^avg.*|^total.*|.*timestamp.*",names(pmltraining))
